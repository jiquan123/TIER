...
class LinearityIQA(nn.Module):
    def __init__(self, arch='resnet18', pool='avg', use_bn_end=False, P6=1, P7=1): # resnext101_32x8d
        ...
        if self.use_bn_end:
            ...
            self.regression = nn.Sequential(nn.Linear(64 * 2 + self.config.hidden_size, 1), nn.BatchNorm1d(1))
        else:
            self.regr6 = nn.Linear(64, 1)
            self.regr7 = nn.Linear(64, 1)
            self.regression = nn.Linear(64 * 2 + self.config.hidden_size, 1)
    ...
    def forward(self, x, ids, mask):
        ...
        text_feature = self.text_encoder(ids, mask)
        f = torch.cat([text_feature, f], 1)
        s = self.regression(f)
        pq.append(s)
